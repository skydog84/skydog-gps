...
# Do NOT commit until tests read 492/492.
...
## 3. sw.js — bump the cache version (now v33 → v34 after Run 6)

index.html changed, so the service worker version MUST move in the same commit.

    line 3:  const CACHE = 'skydog-gps-v33';
      →      const CACHE = 'skydog-gps-v34';
...
```js
T('sw.js cache bumped to v34 (platform-aware AdMob ids ship fresh)', sw.includes("skydog-gps-v34") && !sw.includes("skydog-gps-v33") && !sw.includes("skydog-gps-v32"));
```
...
# 1. tests must be 492/492 — no push before this reads 100%
...
git -c user.name="SkyDog" -c user.email="skydog8426@gmail.com" \
    commit -am "Android AdMob: platform-aware ad ids, sw v34"
git push
...
- [ ] Tests 492/492 BEFORE the push, not after.
...
> ⚠️ Updated 2026-08-01 (Run 6): Run 5 took v32 and the Run 6 Trip Log + Replay build took v33, so this patch now bumps v33 → v34. Numbers above already reflect that.
